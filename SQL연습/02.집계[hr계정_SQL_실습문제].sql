...
-- 예) 2014년 07월 10일
SELECT DATE_FORMAT(MIN(A.hire_date), '%Y년 %m월 %d일') AS 입사일
FROM employees A
...
SELECT MAX(salary) * 12 AS 최고연봉,
	   MIN(salary) * 12 AS 최저연봉
FROM salaries
WHERE to_date = '9999-01-01';

-- 문제6.
-- 최고 어린 사원의 나이와 최 연장자의 나이는?
SELECT MAX(A.birth_date) AS 최저연령,
	   MIN(A.birth_date) AS 최고연령
FROM employees A
INNER JOIN titles B ON A.emp_no = B.emp_no
WHERE B.to_date = '9999-01-01';
